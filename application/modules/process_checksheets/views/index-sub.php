			...
			<div class="card card-stretch shadow card-custom">
				<div class="card-header justify-content-between d-flex flex-wrap align-items-center py-3 px-3 px-md-4 gap-2">
					<h3 class="m-0 font-weight-bolder d-flex align-items-center" style="font-size: 1.15rem;">
						<a href="<?= base_url($this->uri->segment(1)); ?>" title="Back" class="btn btn-light btn-sm btn-icon mr-2 shadow-xs"><i class="fa fa-arrow-left text-dark"></i></a> 
						<span>List Folder</span>
					</h3>
					<?php if ($menus_perm['create'] == '1') : ?>
						<button type="button" class="btn btn-primary btn-sm font-weight-bold px-3" id="add-sub-folder"><i class="fa fa-plus mr-1"></i> New Folder</button>
					<?php endif; ?>
				</div>
					...
					<div class="row align-items-center mb-3">
						...
						<div class="col-12 col-md-7 col-lg-8">
							...
							<nav class="breadcrumb py-2 px-3 m-0 bg-light rounded d-flex flex-wrap align-items-center" style="font-size: 12px; line-height: 1.6;">
								...
								<span class="breadcrumb-item active font-weight-bold text-dark"><?= $parent->name; ?></span>
							</nav>
						</div>
					</div>
								...
								if ($subFolder) foreach ($subFolder as $dt) : $n++; ?>
									<tr>
										<td class="py-2 align-middle">
											<a href="<?= base_url($this->uri->segment(1) . "/?p=" . $dt->process_id . "&sub=" . $dt->id); ?>" class="text-dark">
										...
										<td class="py-2 text-center align-middle">
											<?php if ($menus_perm['update'] == '1') : ?>
												<button type="button" class="btn btn-icon btn-xs btn-warning edit" data-id="<?= $dt->id; ?>" title="Edit"><i class="fa fa-edit"></i></button>
											<?php endif; ?>
											<?php if ($menus_perm['delete'] == '1') : ?>
												<button type="button" class="btn btn-icon btn-xs btn-danger delete" data-id="<?= $dt->id; ?>" title="Hapus"><i class="fa fa-trash"></i></button>
											<?php endif; ?>
										</td>
									</tr>
								<?php endforeach; ?>
