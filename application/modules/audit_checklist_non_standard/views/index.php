							...
							<?php if (!empty($schedules)) foreach ($schedules as $k => $v) : $k++; ?>
								<tr>
									...
									<td class="text-center">
										<?php if (!empty($has_checklist[$v->schedule_id])) : ?>
											<a href="<?= site_url('audit_checklist_non_standard/view/' . $v->schedule_id); ?>" class="btn btn-xs btn-icon btn-info" title="View Checklist">
												<i class="fa fa-eye"></i>
											</a>
											<a href="<?= site_url('audit_checklist_non_standard/create/' . $v->schedule_id); ?>" class="btn btn-xs btn-icon btn-warning" title="Edit Checklist">
												<i class="fa fa-edit"></i>
											</a>
										<?php else : ?>
											<a href="<?= site_url('audit_checklist_non_standard/create/' . $v->schedule_id); ?>" class="btn btn-xs btn-icon btn-primary" title="Create Checklist">
