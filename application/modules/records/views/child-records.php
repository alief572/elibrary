<?php if (($getRecords)) : ?>
  <?php foreach ($getRecords as $form) :
  ...
  ?>
    <tr class="tree-row" data-id="<?= $form->id; ?>" data-parent-id="<?= $parent_id; ?>" data-depth="<?= $depth; ?>" data-loaded="false">
      <td class="py-1">
        <a href="javascript:void(0)" data-id="<?= $form->id; ?>" class="cursor-pointer <?= ($form->flag_type == 'FOLDER') ? 'folder' : 'file'; ?> text-dark">
          <div class="d-flex justify-content-start align-items-center" style="padding-left: <?= $depth * 30 ?>px;">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <?php if ($form->flag_type == 'FILE') : ?>
                ...
                <?php else : ?>
                  <a class="dropdown-item view-record" href="javascript:void(0)" data-id="<?= $form->id; ?>"><i class="<?= isset($icon) ? $icon : 'fas fa-file-pdf' ?> mr-2" style="color: <?= isset($iconColor) ? $iconColor : '#2b579a' ?>;"></i> View Document</a>
                <?php endif; ?>
